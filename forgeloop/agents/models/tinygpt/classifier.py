...
class ClassifierConfig:
    ...
    Attributes:
        ...
        pad_id: Token id used to locate the last non-pad position for pooling.
        head_dropout: Dropout probability applied to the pooled features before the head.
    """
...
class TinyGPTClassifier(nn.Module):
    ...
    def freeze_backbone(self) -> None:
        """Freeze all backbone parameters so only the head (and any unfrozen blocks) train."""
    ...
    def trainable_parameters(self) -> list[nn.Parameter]:
        """Return the parameters with ``requires_grad`` set, for the optimizer.
    ...
    def save(self, path: str | Path) -> None:
        """Write the state dict and backbone/classifier config to ``path``.
    ...
    def load(
        ...
        *,
        ...
        """Rebuild the backbone and classifier from ``path`` and load the saved weights.

        Args:
            path: Directory holding ``config.json`` and ``model.pt``.
            device: Optional target device for the loaded model.

        Returns:
            A restored TinyGPTClassifier in eval mode.
        """
        path = Path(path)
