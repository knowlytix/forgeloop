...
from forgeloop.agents.models.tinygpt.positional import (
    LearnedPositionalEmbedding,
    SinusoidalPositionalEmbedding,
)
from forgeloop.agents.models.tinygpt.transformer_block import TransformerBlock
from forgeloop.agents.models.tinygpt.losses import cross_entropy_loss
...
class TinyGPT(nn.Module):
    ...
    def forward_cached(
        ...
        is a ``KVCache`` from ``agentlab.models.tinygpt.kv_cache`` or
        ...
        """
        from forgeloop.agents.models.tinygpt.kv_cache import KVCache
    ...
    def loss(self, input_ids: torch.Tensor, target_ids: torch.Tensor) -> torch.Tensor:
        """Return the next-token cross-entropy loss for the given inputs and targets.

        Args:
            input_ids: Input token ids, shape (B, T).
            target_ids: Target token ids aligned to the inputs.

        Returns:
            A scalar cross-entropy loss tensor.
        """
    ...
    def generate(
        ...
        """Autoregressively sample tokens from the prompt with temperature and top-k/top-p filtering.

        Args:
            prompt_ids: Prompt token ids as a list or tensor.
            max_new_tokens: Number of tokens to generate.
            temperature: Softmax temperature; lower is greedier.
            top_k: Keep only the top-k logits before sampling, if set.
            top_p: Nucleus sampling threshold on cumulative probability, if set.
            device: Device to run generation on; defaults to the model's device.

        Returns:
            The prompt token ids followed by the generated token ids.
        """
    ...
    def num_parameters(self, exclude_embedding: bool = False) -> int:
        """Count the model's parameters, optionally excluding the token and LM-head embeddings.

        Args:
            exclude_embedding: When True, subtract the embedding parameter count.
