...
class SinusoidalPositionalEmbedding(nn.Module):
    ...
    def positional_table(self, length: int | None = None) -> torch.Tensor:
        """Return a detached copy of the sin/cos table for the first ``length`` positions.

        Args:
            length: Number of positions to return; defaults to ``max_len``.

        Returns:
            A ``(length, embedding_dim)`` tensor.
        """
