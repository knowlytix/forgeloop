...
class LoRALinear(nn.Module):
    ...
    def trainable_parameter_count(self) -> int:
        """Return the number of trainable parameters in the low-rank update (A and B).

        Returns:
            The combined element count of the A and B factors.
        """
...
def trainable_param_count(model: nn.Module) -> int:
    """Return the total number of parameters in ``model`` with ``requires_grad`` set.
