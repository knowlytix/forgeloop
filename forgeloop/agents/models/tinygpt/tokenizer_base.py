...
class BaseTokenizer(abc.ABC):
    ...
    def encode_batch(self, texts: Iterable[str]) -> list[list[int]]:
        """Encode each text in the batch to a list of ids.
    ...
    def decode_batch(self, batches: Iterable[list[int]]) -> list[str]:
        """Decode each id list in the batch back to text.

        Args:
            batches: An iterable of id lists.

        Returns:
            One decoded string per id list.
        """
