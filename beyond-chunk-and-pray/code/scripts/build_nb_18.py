"""Builder for notebooks/18_external_persistence_kal_a_inline.ipynb (Ch15).

Emits a valid nbformat-4 notebook with nbformat. CPU-only: it does NOT
execute the notebook, load the trained store, or run Qwen. Run:

    python scripts/build_nb_15.py
"""
...
md(
    "# Ch16 — Persisting to an external store (KAL / Postgres)\n"
    "\n"
...
# --------------------------------------- Listing 2: persist + read back (mock)
md(
    ...
    "network. `persist_store_to_kal_sync` is the async→sync bridge around the\n"
    "async `insert_triples`. We seed the mock's `fixed_triples` with the converted\n"
    "records so a subsequent `query_triples` reads them back — a full round trip."
)
...
# ----------------------------------- Listing 3: confidence + verification read
md(
...
code(
    ...
    "\n"
    "print(\"Ch16 self-check passed: graph round-trips through KAL intact.\")"
)
...
out = "beyond-chunk-and-pray/code/notebooks/18_external_persistence_kal_a_inline.ipynb"
