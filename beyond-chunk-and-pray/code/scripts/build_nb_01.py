...
"""Builder for notebooks/01_why_chunk_and_pray_fails_a_inline.ipynb.
...
OUT = os.path.join(HERE, "notebooks", "01_why_chunk_and_pray_fails_a_inline.ipynb")
...
cells.append(new_markdown_cell(
    ...
    "\n"
    "A vanilla vector RAG over the Northwind FY2025 annual report. We ask "
    "*\"What was total revenue?\"* The right answer (355.0) lives in a **table "
    "cell**; the dense index, scoring lexical overlap, can prefer a prose "
    "sentence that *says* \"revenue\" but carries **no authoritative number** "
    "and **no provenance**. We then preview the triple-mediated `RagPipeline`, "
    "which returns the ENM-exact figure with a `file:line:char` span.\n"
    "\n"
    "**Grounded, not guessed.** The tiny vector RAG below runs on CPU with a "
    "bag-of-words encoder, so it needs no GPU and no Qwen. The `RagPipeline` "
    "listing is marked for CI execution (it loads the trained store and Qwen)."
))
...
cells.append(new_markdown_cell(
    "## The corpus\n"
    "\n"
    "We load the annual report as plain markdown. Every authoritative number "
    "lives in a **table**; the MD&A / Risk / Outlook sections are prose with no "
    "figures. We point at `data/annual_report.md` relative to the repo root."
))

# Cell 2 — load the corpus text (CPU, no store).
cells.append(new_code_cell(
    "import pathlib\n"
    "\n"
    'DATA = pathlib.Path(os.environ.get("GMS_RAG_DATA", "data"))\n'
...
cells.append(new_markdown_cell(
    ...
    "\n"
    "This is the entire \"chunk and pray\" recipe: split the document on blank "
    "lines, embed each chunk with a deterministic bag-of-words vector, rank by "
    "cosine to the question, take top-k. No external model — the failure mode "
    "is structural, not a quirk of any one embedder."
))

# Cell 3 — the tiny vector RAG implementation.
cells.append(new_code_cell(
    "import math, re\n"
...
cells.append(new_markdown_cell(
    ...
    "\n"
    "We ask the numeric question. The word \"total\" in *\"total revenue\"* "
    "lexically matches the **Balance Sheet** block — \"**Total** Assets\", "
    "\"**Total** Liabilities\" — at least as well as it matches the income-"
    "statement Revenue row. A bag-of-words ranker has no notion that \"total "
    "revenue\" is one concept; it just counts overlapping tokens. So the top hit "
    "is the **wrong table**: it contains 540.0 / 210.0 / 330.0 and **not** the "
    "authoritative 355.0.\n"
    "\n"
    "Expected (grounded in `data/corpus_facts.md`): the top chunk is the Balance "
    "Sheet table near line 45, whose figures do **not** include the answer; the "
    "income-statement Revenue cell (355.0 at line 36) is out-ranked."
))

# Cell 4 — show the wrong/no-provenance retrieval.
cells.append(new_code_cell(
...
cells.append(new_markdown_cell(
    "**What this means.** The vanilla RAG would now hand that *wrong* table to an "
    "LLM and ask it to \"answer from context.\" The model must pick a number from "
    "540.0 / 210.0 / 330.0 — a **silently wrong** figure — or, when handed the "
    "right table, *parse* one cell out of eight with **no cell-level provenance**. "
    "There is no guard against either error. These are three of the four failure "
...
cells.append(new_markdown_cell(
    ...
    "\n"
    "**Marked for CI execution.** This cell loads the trained store and the local "
    "Qwen synthesizer; do not run it during authoring (shared GPU). It returns "
    "the **ENM-exact** total revenue with a `file:line:char` provenance span. The "
    "expected values come from `data/corpus_facts.md`: `lookup_enm("
    '"income_statement", "Revenue/FY2025") == 355.0`, retrieved as the triple '
    "`('revenue', 'has_fy2025', '355.0')` with a span into the income-statement "
    "table."
))

# Cell 5 — RagPipeline preview (CI-only; not executed here).
cells.append(new_code_cell(
...
cells.append(new_markdown_cell(
    "**What this means.** The triple-mediated route never parses a number out of "
    "prose: the figure comes from Exact Numerical Memory (`lookup_enm`), and the "
...
cells.append(new_markdown_cell(
    "## Exercise\n"
    "\n"
    "Break the vector RAG on a **second** numeric question. We ask for net income "
    "(70.0 per `corpus_facts.md`). Here top-1 *is* the income-statement table — "
    "but the chunk holds **eight** numbers across four rows and two years, so "
    "there is no way to return a single, provenance-backed figure. Retrieving the "
    "right table is not the same as answering the question."
))

# Cell 6 — exercise (CPU-only, runs).
cells.append(new_code_cell(
...
cells.append(new_markdown_cell(
    "## Self-check\n"
    "\n"
    "The chapter's claim: a top-k vector retriever, ranking the numeric question "
    "by lexical overlap, does **not** isolate the authoritative table cell — so it "
    "cannot return a single provenance-backed figure. We assert that directly "
    "(CPU-only; no store, no Qwen)."
))

# Final cell — self-check assert (CPU-only).
cells.append(new_code_cell(
